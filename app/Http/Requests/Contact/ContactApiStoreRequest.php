...
class ContactApiStoreRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        if ($this->has('step') && empty($this->step)) {
            $data['step'] = 1;
        }

        if ($this->has('lifes') && empty($this->lifes)) {
            $data['lifes'] = 1;
        }

        if ($this->has('pricingType') && empty($this->pricingType)) {
